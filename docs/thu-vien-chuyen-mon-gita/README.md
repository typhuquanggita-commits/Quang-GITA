# THƯ VIỆN CHUYÊN MÔN GITA

> 🏠 **Chỉ mục toàn hệ:** [`../README.md`](../README.md) — 124 tài liệu · 9 bộ · cấu trúc hai lớp · 11 nguyên tắc chuyên môn · 10 chuẩn an toàn.

### Bộ công cụ tác nghiệp cho Tư vấn – Coach – Trainer – Giáo viên · Phiên bản 1.0

---

## 1. Vị trí của bộ này trong hệ thống

| Bộ tài liệu | Trả lời câu hỏi | Ai dùng |
|---|---|---|
| `leader-boom-365/` | **Dạy cái gì, theo chuẩn nào** | Trainer, ACT, cơ quan quản lý |
| `he-thong-huan-luyen-gita/` | **Vận hành thế nào** — ai làm gì, khi nào, tiền ở đâu | Toàn bộ vận hành |
| **`thu-vien-chuyen-mon-gita/`** (bộ này) | **Xử lý ca cụ thể thế nào** — tra vấn đề, chọn giải pháp, chấm năng lực, in biểu mẫu | Tư vấn, Coach, Trainer, Giáo viên |
| [`an-toan-va-phan-quyen/`](../an-toan-va-phan-quyen/README.md) | **Ai được xem gì, làm gì** — 13 vai trò, ma trận quyền, phân loại dữ liệu, bảo mật | Công nghệ, BLĐ, toàn bộ nhân sự |
| [`nhan-dien-thuong-hieu/`](../nhan-dien-thuong-hieu/README.md) | **Trông và nói như thế nào** — logo, màu, chữ, giọng nói | Truyền thông, Thiết kế |
| [`nhuong-quyen-leader-boom/`](../nhuong-quyen-leader-boom/README.md) | **Chuyển giao cho đơn vị khác thế nào** — chuẩn nhượng quyền, kiểm định | BLĐ, Pháp chế, Đối tác |
| [`cong-dong-leader-boom/`](../cong-dong-leader-boom/README.md) | **Giữ nhịp sau trại thế nào** — cộng đồng, squad, thử thách 21/90 ngày | Coach, Truyền thông |
| [`ho-so-bao-ho/`](../ho-so-bao-ho/README.md) | **Bảo hộ chương trình thế nào** — nhãn hiệu, quyền tác giả, chuẩn trình bày | BLĐ, Pháp chế |
| [`seo-va-hien-dien-so/`](../seo-va-hien-dien-so/README.md) | **Để phụ huynh tìm thấy thế nào** — **220 phác đồ là nguồn nội dung SEO chính** | Truyền thông, Nội dung |

Đây là bộ được mở ra **hằng ngày khi làm việc với một học viên cụ thể**, không phải bộ đọc một lần.

---

## 2. Bộ tài liệu

| # | Tài liệu | Nội dung | Dùng khi |
|---|---|---|---|
| 00 | `README.md` | Cách dùng thư viện, quy tắc tra cứu | Lần đầu |
| 01 | [`01-thu-vien-220-van-de.md`](01-thu-vien-220-van-de.md) | **Danh mục 220 vấn đề** · cấu trúc phác đồ 16 cột · quy tắc chọn đòn bẩy. **Phác đồ đầy đủ ở [`phac-do/`](phac-do/README.md)** | Mọi ca |
| 02 | [`02-khung-nang-luc-18.md`](02-khung-nang-luc-18.md) | **18 năng lực chuẩn** nhóm theo G-I-T-A · 5 cấp độ · rubric · ánh xạ CT GDPT 2018 & CASEL | Chấm năng lực, viết báo cáo |
| 03 | [`03-bo-cong-cu-danh-gia.md`](03-bo-cong-cu-danh-gia.md) | Bộ công cụ đánh giá đầu vào · SWORT GITA · quy tắc 4 nguồn · **rà soát tính hợp lệ của các công cụ đang dùng** | Tiếp nhận học viên mới |
| 04 | [`04-bo-bieu-mau.md`](04-bo-bieu-mau.md) | Danh mục và vai trò **20 biểu mẫu BM-01 → BM-20**. **Nội dung đầy đủ ở TL 12** | Tra mã biểu mẫu |
| 05 | [`05-thu-vien-hoat-dong.md`](05-thu-vien-hoat-dong.md) | Mẫu phiếu hoạt động chuẩn + 53 hoạt động đã rà soát rủi ro (50 đang dùng, 3 đã loại) | Thiết kế buổi huấn luyện |
| 06 | [`06-giao-an-lop-hoc-vien.md`](06-giao-an-lop-hoc-vien.md) | **4 mẫu giáo án lớp — bản tối ưu hoá** · nghi thức lớp · quy trình duyệt | Giáo viên, GV trưởng |
| 07 | [`07-dong-hanh-phu-huynh.md`](07-dong-hanh-phu-huynh.md) | Hệ đồng hành phụ huynh 365 ngày · đánh giá gia đình · 20 vấn đề nhóm N07 | Coach, Tư vấn |
| 08 | [`08-nen-tang-so-va-ai.md`](08-nen-tang-so-va-ai.md) | 9 vai trò người dùng · 18 trung tâm chức năng · phân quyền theo gói · **ranh giới AI và con người** | BLĐ, Công nghệ |
| 09 | [`09-san-pham-5-goi.md`](09-san-pham-5-goi.md) | 5 gói dịch vụ · ranh giới · quyền lợi · tiêu chuẩn chuyển tầng · khuyến nghị gói | Tư vấn, Kinh doanh |
| 10 | [`10-quan-tri-chat-luong.md`](10-quan-tri-chat-luong.md) | 8 điểm kiểm soát chất lượng · bộ KPI toàn hệ · quy trình kiểm tra định kỳ | BLĐ |
| **11** | [`11-rubric-hanh-vi-18x5.md`](11-rubric-hanh-vi-18x5.md) | **Đủ 90 ô** — 18 năng lực × 5 cấp, hành vi quan sát được + bằng chứng chấp nhận + sai lầm chấm | **Thước để chấm năng lực** |
| **12** | [`12-bieu-mau-day-du.md`](12-bieu-mau-day-du.md) | **20 biểu mẫu nội dung đầy đủ**, in ra điền tay được · ví dụ KPI đã điền | In ấn, tác nghiệp |
| **13** | [`13-ngan-hang-cau-hoi.md`](13-ngan-hang-cau-hoi.md) | **Ngân hàng câu hỏi CC-01 → CC-14** · 30 câu × 3 bản tuổi · kịch bản phỏng vấn · quy trình đối chiếu 4 nguồn | Đánh giá |
| **14** | [`14-the-hoat-dong-day-du.md`](14-the-hoat-dong-day-du.md) | **50 thẻ hoạt động** — lời dẫn mẫu, câu hỏi đúc kết, an toàn, quyền được dừng | Trainer dẫn buổi |
| **15** | [`15-chuong-trinh-phu-huynh-90-ngay.md`](15-chuong-trinh-phu-huynh-90-ngay.md) | **13 tuần cho cha mẹ** — mỗi tuần một kỹ năng, một việc 10 phút, một điều cần bỏ | Coach dẫn gia đình |
| **16** | [`16-nen-tang-so-chi-tiet.md`](16-nen-tang-so-chi-tiet.md) | 🔧 **Đặc tả nền tảng số** — 7 nhóm chức năng · phân quyền · nguyên tắc dùng AI | Công nghệ |
| **17** | [`17-bang-tra-cuu-220.md`](17-bang-tra-cuu-220.md) | 🔧 **Bảng tra cứu 220 phác đồ** — tra theo lời phụ huynh · theo nhóm · ngược từ năng lực | **Coach khi có ca** |
| **18** | [`18-nguong-theo-dai-tuoi.md`](18-nguong-theo-dai-tuoi.md) | 🔧 **Ngưỡng theo 3 dải tuổi** — hệ số quy đổi · công cụ đo theo dải · quy trình hiệu chuẩn | **Coach khi nghiệm thu** |
| **★** | [`phac-do/`](phac-do/README.md) | **220 phác đồ đầy đủ** — 11 tệp theo nhóm N01–N11, mỗi phác đồ 16 lớp | **Mọi ca cụ thể** |

> **Hai lớp tài liệu.** Tài liệu **01–10** là **bản tóm tắt điều hành** — khung, nguyên lý, danh mục mã.
> Tài liệu **11–15** và thư mục **`phac-do/`** là **bản tác nghiệp** — thứ mở ra khi làm việc với một
> học viên cụ thể. **Khi hai bản khác nhau, bản tác nghiệp là bản đúng.**
>
> | Bản tóm tắt | → | Bản tác nghiệp |
> |---|---|---|
> | `01` Danh mục 220 vấn đề | → | [`phac-do/`](phac-do/README.md) 220 phác đồ đầy đủ |
> | `02` Khung 18 năng lực | → | [`11`](11-rubric-hanh-vi-18x5.md) Rubric 90 ô |
> | `03` Bộ công cụ đánh giá | → | [`13`](13-ngan-hang-cau-hoi.md) Ngân hàng câu hỏi |
> | `04` Danh mục 20 biểu mẫu | → | [`12`](12-bieu-mau-day-du.md) Nội dung biểu mẫu |
> | `05` Thư viện hoạt động | → | [`14`](14-the-hoat-dong-day-du.md) 50 thẻ hoạt động |
> | `07` Đồng hành phụ huynh | → | [`15`](15-chuong-trinh-phu-huynh-90-ngay.md) Chương trình 13 tuần |

---

## 3. Quy trình tra cứu một ca — bốn bước

```
BƯỚC 1 · ĐỌC DỮ LIỆU
Có đủ 4 nguồn chưa? (HS tự đánh giá · phụ huynh · dữ liệu hành vi · bằng chứng kết quả)
Nếu chưa đủ → quay lại thu dữ liệu. KHÔNG tra thư viện khi dữ liệu chưa đủ.
        ↓
BƯỚC 2 · ĐỊNH VỊ VẤN ĐỀ  → TL 01
Xác định biểu hiện chính thuộc nhóm nào (N01–N11), vấn đề số mấy.
Một ca thường chạm 3–6 vấn đề. Ghi hết, chưa xử lý.
        ↓
BƯỚC 3 · ĐỌC CƠ CHẾ  → TL 01 §3 (ma trận 8×8) + TL 03
Vấn đề nào là chính, phụ, duy trì? Đâu là ĐIỂM ĐÒN BẨY?
        ↓
BƯỚC 4 · CHỌN PHÁC ĐỒ  → TL 01
Lấy phác đồ của 1–3 vấn đề đòn bẩy. KHÔNG lấy cả 6.
Chọn đúng cột lộ trình theo tầng học viên đang ở (7 / 21 / 90 / 365 / 365 NC).
```

> **Quy tắc số một của thư viện:** giá trị của GITA **không** nằm ở việc sở hữu nhiều giải pháp,
> mà ở năng lực **xác định đúng vấn đề, đọc đúng cơ chế, chọn đúng đòn bẩy, đưa đúng giải pháp
> vào đúng thời điểm, và dịch chuyển quyền phát triển về cho học viên**.

---

## 4. Sáu lỗi tra cứu thường gặp

| Lỗi | Hệ quả | Cách tránh |
|---|---|---|
| Tra thư viện khi mới có 1 nguồn dữ liệu | Chọn sai vấn đề, giải pháp đi sai hướng ngay từ đầu | Kiểm đủ 4 nguồn trước |
| Lấy hết phác đồ của mọi vấn đề tìm thấy | Học viên quá tải, bỏ cuộc trong 2 tuần | Tối đa 3 đòn bẩy |
| Lấy phác đồ 90 ngày cho học viên mới ở tầng 7 ngày | Can thiệp khi chưa hiểu cơ chế | Chọn cột đúng tầng |
| Đọc "nguyên nhân cốt lõi" trong bảng như kết luận | Bỏ qua đặc thù của ca | Bảng cho **giả thuyết**, không cho kết luận |
| Không ghi bằng chứng phản bác | Hồ sơ tự khẳng định, không kiểm chứng | Bắt buộc ghi cả hai chiều |
| Copy nguyên văn phác đồ vào báo cáo gửi gia đình | Gia đình đọc thấy chung chung, mất niềm tin | Viết lại bằng dữ liệu của chính con họ |

---

## 5. Năng lực chuyên môn được chứng minh bằng gì

Một Coach giỏi **không** chứng minh năng lực bằng việc nói nhiều, đưa nhiều bài tập,
hay giữ học viên lâu trong chương trình. Chứng minh bằng một chuỗi rất rõ:

1. **Dữ liệu ngày càng chính xác hơn.**
2. **Giả thuyết ngày càng ít nhưng đúng hơn.**
3. **Giải pháp ngày càng tập trung hơn.**
4. **Học viên ngày càng chủ động hơn.**
5. **Phụ huynh ngày càng ít phải kiểm soát hơn.**
6. **Kết quả ngày càng bền khi mức hỗ trợ bên ngoài giảm xuống.**

Đây phải trở thành bản sắc của GITA — và là tiêu chí đánh giá đội ngũ chuyên môn.
