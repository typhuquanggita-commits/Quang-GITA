...
export async function docGiaHienHanh(db) {
  const r = await db.prepare(
    ...
    ' FROM bangGia ORDER BY ghiLuc ASC').all();
...
export async function soDoiGia(y, env, db, hoSo) {
  ...
  const r = tang
    ? await db.prepare('SELECT * FROM bangGia WHERE tang = ? ORDER BY ghiLuc ASC')
        .bind(tang).all()
    : await db.prepare('SELECT * FROM bangGia ORDER BY ghiLuc ASC').all();
  const ds = (r.results || r || []);
