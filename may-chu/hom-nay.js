...
export async function docCheDoBao(db, maNha) {
  const r = await db.prepare(
    'SELECT bat, ghiLuc FROM cheDoBao WHERE maNha = ? ORDER BY ghiLuc DESC LIMIT 1')
...
export async function docGhimCon(y, env, db, hoSo) {
  ...
  const r = await db.prepare(
    'SELECT o, ghiChu, ghiLuc FROM ghimCon WHERE maCon = ? ORDER BY ghiLuc ASC')
    .bind(maCon).all();
...
export async function docDongYAnhCon(db, maCon) {
  const r = await db.prepare(
    'SELECT dongY, ghiLuc FROM dongYAnhCon WHERE maCon = ? ORDER BY ghiLuc DESC LIMIT 1')
