...
export async function bayConSoCEO(y, env, db, hoSo) {
  ...
  const thang = String(bayGio).slice(0, 7);
  ...
  const nha = await db.prepare(
    'SELECT maKhachHang, vaoLuc, trangThai FROM hoSoKhach').bind().all();
  ...
  const dtThang = await db.prepare(
    "SELECT COALESCE(SUM(soTien),0) t FROM phieuThu" +
    " WHERE trangThai = 'daDuyet' AND substr(ghiLuc,1,7) = ?").bind(thang).first();
  ...
  const chi3 = await db.prepare(
    "SELECT COALESCE(SUM(soTien),0) t FROM chiPhi WHERE trangThai = 'daDuyet'" +
    " AND ngayChi >= date(?, '-3 months')").bind(String(bayGio).slice(0, 10)).first();
  const chiThang = Math.round(Number((chi3 || {}).t || 0) / 3);
