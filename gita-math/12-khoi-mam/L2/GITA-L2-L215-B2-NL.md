---
ma: "GITA-L2-L215-B2-NL"
khoi: "L2"
khoi_ten: "Lớp 2"
chu_de: "L215"
chu_de_ten: "Biểu đồ tranh"
mach: "T"
mach_ten: "Thống kê và Xác suất"
buoi: "2"
thoi_luong: "40"
thang_diem: "40"
chuan: "Chương trình GDPT 2018 — môn Toán lớp 2"
cap_nhat: "2026-08-30"
---

# Biểu đồ tranh — buổi 2/5

**HỌC VIỆN PHÁT TRIỂN TÀI NĂNG TOÀN CẦU — GITA** · gita.edu.vn  
Lớp 2 · 7 – 8 tuổi · Mã: **GITA-L2-L215-B2-NL**  
Mạch: **Thống kê và Xác suất** *(Cambridge: Statistics and Probability)*  
Thời lượng: **40 phút** · Thang điểm: **40**  
Bản dành cho: **người lớn ngồi cùng**

---

## YÊU CẦU CẦN ĐẠT CỦA BUỔI NÀY

*Theo Chương trình GDPT 2018 — môn Toán lớp 2.*

- Đọc được biểu đồ tranh đơn giản
- Trả lời câu hỏi từ biểu đồ tranh
- Vẽ được biểu đồ tranh từ bảng số liệu

## TƯ DUY BUỔI NÀY RÈN

*Theo khung Thinking and Working Mathematically của Cambridge Primary Mathematics.*

- **Nói cho người khác tin** *(Convincing)* — Giải thích cách nghĩ của mình để người khác hiểu và tin.  
  Câu người lớn hỏi: “Vì sao con biết? Con nói cho bạn hiểu đi.”
- **Xem chỗ nào chưa ổn** *(Critiquing)* — Nhận ra chỗ tốt và chỗ chưa tốt trong bài mình hoặc bài bạn.  
  Câu người lớn hỏi: “Bạn làm thế này, con thấy có chỗ nào chưa ổn không?”

## ĐỒ VẬT CẦN CHUẨN BỊ TRƯỚC

- Que tính xếp thành nhóm bằng nhau
- Giấy kẻ ô
- Bút màu
- Giấy kẻ ô để vẽ biểu đồ tranh
- Bảng số liệu in sẵn
- Một hộp bi có bi hai màu để thử thật

---

## PHẦN A — Khởi động — nhắc lại buổi trước

**Bảng nhân, bảng chia 2 và 5**

> **Cô/mẹ đọc lên:** Con đọc to bảng nhân một lượt trước khi làm.

**Đáp án từng việc**

a) 5 × 8 = …
   → **40**

b) 14 : 2 = …
   → **7**

c) Viết phép nhân thay cho phép cộng: 2 + 2 + 2 + 2
   → **2 × 4 = 8**

d) 25 : 5 = …
   → **5**

e) 2 × 1 = …
   → **2**

**Hỏi thêm một câu** *(rèn: Rút ra quy luật · Cho một ví dụ)*: Con biết 2 × 6 rồi. Vậy 6 × 2 bằng mấy? Có phải lúc nào cũng thế không?

**Thế nào là con đã hiểu:** Trẻ hiểu phép nhân là cộng các số hạng bằng nhau, và dùng được phép nhân đã thuộc để suy ra phép chia tương ứng.

**Con tắc thì làm gì:** Xếp que tính thành từng nhóm rồi đếm — thấy được mới nhớ được.

**Nếu con làm nhanh:** Hỏi 2 × 11, một phép ngoài bảng, xem trẻ có tự cộng thêm 2 không.


---

## PHẦN B — Học cái mới

**Kiểm đếm về con vật nuôi ở nhà**

> **Cô/mẹ đọc lên:** Con đi hỏi từng bạn rồi ghi ngay vào bảng, đừng để nhớ trong đầu.

**Đáp án từng việc**

a) Con đi hỏi và kiểm đếm về con vật nuôi ở nhà. Ghi vào bảng.
   → **trẻ tự thu thập — người lớn kiểm bảng có đủ cột và đủ ô không**

b) Loại nào nhiều nhất trong bảng của con?
   → **trẻ đọc từ bảng của mình**

c) Con vẽ biểu đồ tranh cho bảng ấy, mỗi hình thay cho một bạn.
   → **trẻ tự vẽ — số hình mỗi hàng phải khớp với bảng**

d) Nhìn biểu đồ, con nói một điều con thấy được.
   → **bất kỳ nhận xét nào đọc đúng từ biểu đồ**

**Hỏi thêm một câu** *(rèn: Đoán thử · Xem chỗ nào chưa ổn)*: Nếu hỏi lớp khác thì bảng có giống thế này không? Con đoán xem.

**Thế nào là con đã hiểu:** Trẻ ghi ngay khi hỏi được, và bảng của trẻ có đủ cột. Số liệu tự đi thu thập nhớ lâu hơn hẳn số liệu cho sẵn.

**Con tắc thì làm gì:** Làm mẫu với ba bạn đầu tiên.

**Nếu con làm nhanh:** Hỏi trẻ tự nghĩ một câu hỏi để đi kiểm đếm.


---

## PHẦN C — Luyện tay

**Bảng kiểm đếm và biểu đồ tranh**

> **Cô/mẹ đọc lên:** Con đọc kỹ bảng trước khi trả lời. Mỗi câu hỏi hỏi một điều khác nhau.

**Đáp án từng việc**

a) Bảng kiểm đếm: quả lê: 4 · quả chuối: 2 · quả táo: 3 · quả xoài: 3. Loại nào nhiều nhất?
   → **quả lê**

b) Loại nào ít nhất?
   → **quả chuối**

c) Có tất cả bao nhiêu quả?
   → **12**

d) Quả lê nhiều hơn quả chuối mấy quả?
   → **2**

e) Nếu mỗi hình vẽ trong biểu đồ tranh thay cho 1 quả thì hàng quả lê vẽ mấy hình?
   → **4**

**Hỏi thêm một câu** *(rèn: Xếp thành nhóm · Chỉ ra chỗ giống nhau)*: Nhìn bảng này con thấy điều gì đáng chú ý? Con kể cho cô nghe.

**Thế nào là con đã hiểu:** Trẻ trả lời đúng câu hỏi được hỏi, không đọc bừa một con số trong bảng. Đây là lỗi mất điểm phổ biến nhất của cả mạch Thống kê ở tiểu học.

**Con tắc thì làm gì:** Che bớt các cột chưa cần, chỉ để lại cột đang hỏi.

**Nếu con làm nhanh:** Cho trẻ tự đi kiểm đếm một thứ có thật trong nhà rồi vẽ biểu đồ.


---

## PHẦN D — Bài toán có lời văn

**Đọc bảng số liệu**

> **Cô/mẹ đọc lên:** Mỗi câu hỏi một điều khác nhau. Con đọc kỹ câu hỏi trước khi nhìn vào bảng.

**Đáp án từng việc**

a) Số quyển sách bạn Nam đọc trong tuần — thứ Hai: 5, thứ Ba: 12, thứ Tư: 11, thứ Năm: 14, thứ Sáu: 6. Ngày nào đọc nhiều nhất?
   → **thứ Năm**

b) Cả tuần đọc bao nhiêu quyển?
   → **48**

c) Thứ Hai và thứ Ba đọc tất cả mấy quyển?
   → **17**

d) Ngày đọc nhiều nhất hơn ngày đọc ít nhất mấy quyển?
   → **9**

**Hỏi thêm một câu** *(rèn: Chỉ ra chỗ giống nhau · Làm cách gọn hơn)*: Câu nào con phải cộng nhiều số? Có cách cộng nào nhanh hơn không?

**Thế nào là con đã hiểu:** Trẻ trả lời đúng **câu được hỏi**, không đọc bừa một con số trong bảng. Đây là lỗi mất điểm phổ biến nhất của mạch Thống kê.

**Con tắc thì làm gì:** Che các cột không liên quan tới câu đang hỏi.

**Nếu con làm nhanh:** Hỏi một câu mà bảng không trả lời được, xem trẻ có nhận ra không.


---

## PHẦN E — Thử thách vui

**Chắc chắn, có thể, không thể**

> **Cô/mẹ đọc lên:** Con nghĩ kỹ rồi chọn một trong ba từ: chắc chắn, có thể, không thể.

**Đáp án từng việc**

a) “Bốc được viên bi đỏ từ hộp có cả bi đỏ và bi xanh” — chắc chắn, có thể hay không thể?
   → **có thể**

b) “Hôm nay con được điểm 10” — chắc chắn, có thể hay không thể?
   → **có thể**

c) “Con mèo biết nói tiếng người” — chắc chắn, có thể hay không thể?
   → **không thể**

d) “Bốc được viên bi đỏ từ hộp chỉ toàn bi xanh” — chắc chắn, có thể hay không thể?
   → **không thể**

e) “Ngày mai trời mưa” — chắc chắn, có thể hay không thể?
   → **có thể**

f) “Tháng sau có ngày thứ Hai” — chắc chắn, có thể hay không thể?
   → **chắc chắn**

**Hỏi thêm một câu** *(rèn: Đoán thử · Nói cho người khác tin)*: Vì sao con chọn từ đó? Con cho cô một ví dụ nữa cùng loại.

**Thế nào là con đã hiểu:** Trẻ phân biệt được “không thể” với “khó xảy ra”. Đây là chỗ nhầm nhiều nhất, và cũng là gốc của khái niệm xác suất sau này.

**Con tắc thì làm gì:** Thử thật bằng hộp bi. Bốc mấy lần cho trẻ thấy.

**Nếu con làm nhanh:** Cho trẻ tự nghĩ ba câu, mỗi câu một mức, rồi đố lại người lớn.


---

## BỐN ĐIỀU GIỮ CHO CON THÍCH TOÁN

1. **Mỗi buổi mở bằng một trò chơi hoặc một câu đố, không mở bằng bài tập.** — Ấn tượng của ba phút đầu quyết định trẻ ngồi vào bàn với tâm thế nào.
2. **Mỗi buổi có ít nhất một việc dùng **đồ vật thật** — que tính, hột hạt, cúc áo, ngón tay — chứ không chỉ có giấy bút.** — Ở tuổi này khái niệm số hình thành qua tay trước khi hình thành qua mắt.
3. **Mỗi buổi kết bằng một việc trẻ **chắc chắn làm được**.** — Buổi học kết thúc ở cảm giác nào thì trẻ nhớ cảm giác ấy, không nhớ nội dung.
4. **Không dùng bút đỏ, không xếp hạng, không so sánh trẻ này với trẻ khác. Sai thì ghi lại để buổi sau làm lại, không ghi điểm trừ.** — Trẻ sợ sai sẽ ngừng đoán, mà ngừng đoán là ngừng học toán.

## ĐỐI CHIẾU VỚI CAMBRIDGE

Khối này tương ứng **Cambridge Stage 2**. Phần trùng nhau: Số tới 1 000, cộng trừ có nhớ, làm quen nhân chia, đơn vị đo, thu thập số liệu và biểu đồ tranh, ngôn ngữ khả năng xảy ra.

*Cambridge đi sớm hơn ở:* Stage 2 làm quen phân số đơn giản (một nửa, một phần tư) sớm hơn chương trình Việt Nam.

*Chương trình Việt Nam đi sớm hơn ở:* Bảng nhân 2, 5 và bảng chia 2, 5 được yêu cầu thuộc lòng, chặt hơn Cambridge ở cùng độ tuổi.

---

*Buổi học do bộ sinh GITA biên soạn ngày 2026-08-30. Mọi đáp số do mã tính ra. Bám Chương trình GDPT 2018 — môn Toán lớp 2, bổ sung khung Thinking and Working Mathematically của Cambridge Primary Mathematics.*
