---
ma: "GITA-L2-L205-B1-NL"
khoi: "L2"
khoi_ten: "Lớp 2"
chu_de: "L205"
chu_de_ten: "Phép nhân và bảng nhân 2, bảng nhân 5"
mach: "S"
mach_ten: "Số và phép tính"
buoi: "1"
thoi_luong: "40"
thang_diem: "40"
chuan: "Chương trình GDPT 2018 — môn Toán lớp 2"
cap_nhat: "2026-08-30"
---

# Phép nhân và bảng nhân 2, bảng nhân 5 — buổi 1/5

**HỌC VIỆN PHÁT TRIỂN TÀI NĂNG TOÀN CẦU — GITA** · gita.edu.vn  
Lớp 2 · 7 – 8 tuổi · Mã: **GITA-L2-L205-B1-NL**  
Mạch: **Số và phép tính** *(Cambridge: Number)*  
Thời lượng: **40 phút** · Thang điểm: **40**  
Bậc: **Làm quen** — Người lớn làm mẫu trước, trẻ làm theo từng bước.  
Bản dành cho: **người lớn ngồi cùng**

---

## YÊU CẦU CẦN ĐẠT CỦA BUỔI NÀY

*Theo Chương trình GDPT 2018 — môn Toán lớp 2.*

- Nhận biết phép nhân là phép cộng các số hạng bằng nhau
- Thuộc bảng nhân 2 và bảng nhân 5
- Giải bài toán có một phép nhân

## TƯ DUY BUỔI NÀY RÈN

*Theo khung Thinking and Working Mathematically của Cambridge Primary Mathematics.*

- **Rút ra quy luật** *(Generalising)* — Nói được điều gì luôn đúng, không chỉ đúng với một trường hợp.  
  Câu người lớn hỏi: “Lúc nào cũng thế à? Con nói lại thành một câu xem.”
- **Cho một ví dụ** *(Specialising)* — Nêu được một trường hợp cụ thể đúng với quy luật đang xét.  
  Câu người lớn hỏi: “Con cho cô một ví dụ nữa được không?”

## ĐỒ VẬT CẦN CHUẨN BỊ TRƯỚC

- Que tính xếp thành nhóm bằng nhau
- Que tính
- Đĩa hoặc vòng để chia nhóm
- Hột hạt hoặc nắp chai để xếp thành nhóm bằng nhau
- Bảng nhân 2 và bảng nhân 5 dán trên tường
- Que tính hoặc hột hạt để chia thử
- Không cần gì

---

## PHẦN A — Khởi động — nhắc lại buổi trước

**Bảng nhân, bảng chia 2 và 5**

> **Cô/mẹ đọc lên:** Con đọc to bảng nhân một lượt trước khi làm.

**Đáp án từng việc**

a) 5 × 6 = …
   → **30**

b) 18 : 2 = …
   → **9**

c) Viết phép nhân thay cho phép cộng: 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2
   → **2 × 10 = 20**

d) 10 : 5 = …
   → **2**

**Hỏi thêm một câu** *(rèn: Rút ra quy luật · Cho một ví dụ)*: Con biết 2 × 6 rồi. Vậy 6 × 2 bằng mấy? Có phải lúc nào cũng thế không?

**Thế nào là con đã hiểu:** Trẻ hiểu phép nhân là cộng các số hạng bằng nhau, và dùng được phép nhân đã thuộc để suy ra phép chia tương ứng.

**Con tắc thì làm gì:** Xếp que tính thành từng nhóm rồi đếm — thấy được mới nhớ được.

**Nếu con làm nhanh:** Hỏi 2 × 11, một phép ngoài bảng, xem trẻ có tự cộng thêm 2 không.


---

## PHẦN B — Học cái mới

**Từ phép cộng sang phép nhân**

> **Cô/mẹ đọc lên:** Con xếp que tính thành từng nhóm bằng nhau rồi mới viết.

**Đáp án từng việc**

a) Xếp 3 nhóm, mỗi nhóm 2 que tính. Con viết phép cộng rồi viết phép nhân tương ứng.
   → **2 + 2 + 2 = 2 × 3 = 6**

b) Xếp 3 nhóm, mỗi nhóm 5 que tính. Con viết phép cộng rồi viết phép nhân tương ứng.
   → **5 + 5 + 5 = 5 × 3 = 15**

c) Xếp 7 nhóm, mỗi nhóm 5 que tính. Con viết phép cộng rồi viết phép nhân tương ứng.
   → **5 + 5 + 5 + 5 + 5 + 5 + 5 = 5 × 7 = 35**

d) Xếp 4 nhóm, mỗi nhóm 2 que tính. Con viết phép cộng rồi viết phép nhân tương ứng.
   → **2 + 2 + 2 + 2 = 2 × 4 = 8**

**Hỏi thêm một câu** *(rèn: Rút ra quy luật · Cho một ví dụ)*: Phép cộng nào cũng viết thành phép nhân được không? Vì sao?

**Thế nào là con đã hiểu:** Trẻ nhận ra chỉ **cộng các số hạng bằng nhau** mới viết thành phép nhân được. Đây là chỗ hiểu bản chất, không phải chỗ học thuộc bảng.

**Con tắc thì làm gì:** Xếp hộ hai nhóm đầu, để trẻ xếp tiếp.

**Nếu con làm nhanh:** Cho một phép cộng có số hạng không bằng nhau, hỏi có viết thành phép nhân được không.


---

## PHẦN C — Luyện tay

**Bảng nhân, bảng chia và số còn thiếu**

> **Cô/mẹ đọc lên:** Nhân là cộng nhiều lần bằng nhau. Chia là chia đều ra. Con nhớ được phép nhân thì suy ra phép chia.

**Đáp án từng việc**

a) Tìm số còn thiếu: 2 × … = 8
   → **4**

b) Có 6 quyển vở chia đều vào 2 hộp. Mỗi hộp mấy quyển vở?
   → **6 : 2 = 3 quyển vở**

c) 5 × 7 = …
   → **35**

d) 15 : 5 = …
   → **3**

e) 35 : 5 = …
   → **7**

**Hỏi thêm một câu** *(rèn: Rút ra quy luật · Làm cách gọn hơn)*: Con biết 5 × 6 = 30. Vậy 30 : 5 bằng mấy? Vì sao con biết ngay mà không cần chia?

**Thế nào là con đã hiểu:** Trẻ dùng phép nhân để tìm ra phép chia thay vì chia lại từ đầu. Nhìn ra **nhân và chia là hai mặt của một việc** tiết kiệm cho trẻ đúng một nửa số bảng phải học.

**Con tắc thì làm gì:** Xếp hột hạt thành từng nhóm bằng nhau, đếm nhóm rồi đếm cả.

**Nếu con làm nhanh:** Hỏi 2 × 5 và 5 × 2 — hai phép cho cùng một kết quả, vì sao?


---

## PHẦN D — Bài toán có lời văn

**Bài toán nhân, chia**

> **Cô/mẹ đọc lên:** Con đọc đề rồi hỏi mình: đây là gộp nhiều nhóm bằng nhau, hay chia đều ra?

**Đáp án từng việc**

a) Mỗi hộp có 2 con cá. Hỏi 4 hộp có mấy con?
   → **8 con**

b) Có 14 cái bánh chia đều vào 2 hộp. Mỗi hộp có mấy cái?
   → **7 cái**

c) Mỗi hộp có 2 quyển vở. Hỏi 8 hộp có mấy quyển?
   → **16 quyển**

d) Có 18 con vịt chia đều vào 2 hộp. Mỗi hộp có mấy con?
   → **9 con**

**Hỏi thêm một câu** *(rèn: Cho một ví dụ · Rút ra quy luật)*: Bài này nhân hay chia? Vì sao con biết?

**Thế nào là con đã hiểu:** Trẻ nhận ra dấu hiệu “mỗi … có …, hỏi tất cả” là nhân, còn “chia đều” là chia — mà không cần thuộc lòng danh sách từ khoá.

**Con tắc thì làm gì:** Chia thật bằng hột hạt vào các đĩa.

**Nếu con làm nhanh:** Cho bài chia có dư để trẻ thấy không phải lúc nào cũng chia hết.


---

## PHẦN E — Thử thách vui

**Đố vui nhân chia**

> **Cô/mẹ đọc lên:** Cuối buổi mình đố nhau. Con nhẩm miệng thôi, không phải viết.

**Đáp án từng việc**

a) 7 con mèo có tất cả mấy cái chân?
   → **28 cái chân**

b) Một đôi dép có 2 chiếc. 20 chiếc dép là mấy đôi?
   → **10 đôi**

c) Chia đều 20 cái kẹo cho 5 bạn. Mỗi bạn mấy cái?
   → **4 cái**

d) Con có 4 viên bi. Anh có gấp 2 lần con. Anh có mấy viên?
   → **8 viên**

**Hỏi thêm một câu** *(rèn: Rút ra quy luật · Xem chỗ nào chưa ổn)*: Câu này con nhân hay chia? Chỗ nào trong đề cho con biết?

**Thế nào là con đã hiểu:** Trẻ nghe 'gấp mấy lần' là nghĩ ngay tới phép nhân, nghe 'chia đều' là nghĩ tới phép chia — mà không cần thử cả hai.

**Con tắc thì làm gì:** Vẽ nhanh sơ đồ: mấy nhóm, mỗi nhóm mấy cái.

**Nếu con làm nhanh:** Hỏi ngược: 'mỗi bạn được 4 cái, chia cho 3 bạn thì cần mấy cái?'


---

## BỐN ĐIỀU GIỮ CHO CON THÍCH TOÁN

1. **Mỗi buổi mở bằng một trò chơi hoặc một câu đố, không mở bằng bài tập.** — Ấn tượng của ba phút đầu quyết định trẻ ngồi vào bàn với tâm thế nào.
2. **Mỗi buổi có ít nhất một việc dùng **đồ vật thật** — que tính, hột hạt, cúc áo, ngón tay — chứ không chỉ có giấy bút.** — Ở tuổi này khái niệm số hình thành qua tay trước khi hình thành qua mắt.
3. **Mỗi buổi kết bằng một việc trẻ **chắc chắn làm được**.** — Buổi học kết thúc ở cảm giác nào thì trẻ nhớ cảm giác ấy, không nhớ nội dung.
4. **Không dùng bút đỏ, không xếp hạng, không so sánh trẻ này với trẻ khác. Sai thì ghi lại để buổi sau làm lại, không ghi điểm trừ.** — Trẻ sợ sai sẽ ngừng đoán, mà ngừng đoán là ngừng học toán.

## ĐỐI CHIẾU VỚI CAMBRIDGE

Khối này tương ứng **Cambridge Stage 2**. Phần trùng nhau: Số tới 1 000, cộng trừ có nhớ, làm quen nhân chia, đơn vị đo, thu thập số liệu và biểu đồ tranh, ngôn ngữ khả năng xảy ra.

*Cambridge đi sớm hơn ở:* Stage 2 làm quen phân số đơn giản (một nửa, một phần tư) sớm hơn chương trình Việt Nam.

*Chương trình Việt Nam đi sớm hơn ở:* Bảng nhân 2, 5 và bảng chia 2, 5 được yêu cầu thuộc lòng, chặt hơn Cambridge ở cùng độ tuổi.

---

*Buổi học do bộ sinh GITA biên soạn ngày 2026-08-30. Mọi đáp số do mã tính ra. Bám Chương trình GDPT 2018 — môn Toán lớp 2, bổ sung khung Thinking and Working Mathematically của Cambridge Primary Mathematics.*
