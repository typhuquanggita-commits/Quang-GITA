# BẢN ĐỒ TỔNG HỢP KIẾN THỨC — LỚP 5 — HỌC KỲ II

**HỌC VIỆN GITA** · *Tư duy xuất sắc, Bản lĩnh dẫn đầu*

- Số mạch kiến thức: **5**
- Phạm vi phiếu: **P055 – P100**
- Cách dùng: treo tường hoặc dán vào trang đầu vở. Trước mỗi mốc kiểm tra,
  học sinh tự đánh dấu ✔ vào mạch đã chắc, ✘ vào mạch còn yếu, rồi làm lại
  đúng những phiếu được liệt kê ở cột **Phiếu ôn lại**.

**Mốc kiểm tra của kỳ này:** Giữa kỳ II (sau phiếu P078) · Cuối kỳ II (sau phiếu P100)

---

## 1. CÂY KIẾN THỨC

```
KIẾN THỨC
├── [F] Hình tròn
│   ├─ Bán kính, đường kính
│   ├─ Chu vi hình tròn
│   └─ Diện tích hình tròn
├── [F] Hình hộp chữ nhật và hình lập phương
│   ├─ Nhận dạng, khai triển hình
│   ├─ Diện tích xung quanh, diện tích toàn phần
│   ├─ Thể tích
│   └─ Đơn vị đo thể tích
├── [E] Số đo thời gian
│   ├─ Bảng đơn vị đo thời gian
│   └─ Cộng, trừ, nhân, chia số đo thời gian
├── [D] Toán chuyển động đều
│   ├─ Vận tốc, quãng đường, thời gian
│   ├─ Chuyển động cùng chiều, ngược chiều
│   └─ Chuyển động trên dòng nước
└── [D] Ôn tập cuối cấp tiểu học
    ├─ Bốn phép tính với số tự nhiên, phân số, số thập phân
    ├─ Toán điển hình
    ├─ Hình học phẳng và không gian
    └─ Biểu đồ và số liệu
```

---

## 2. MẠCH KIẾN THỨC NỐI SANG HỆ THỐNG PHIẾU

### 1. Hình tròn  ·  nhóm **F — Hình học**

**Nội dung cốt lõi:** Bán kính, đường kính · Chu vi hình tròn · Diện tích hình tròn

| | Mã phiếu ôn lại |
|---|---|
| Tuyến 1 | `P077` |
| Tuyến 2 | `P059`, `P064`, `P071`, `P077`, `P088`, `P092` |

### 2. Hình hộp chữ nhật và hình lập phương  ·  nhóm **F — Hình học**

**Nội dung cốt lõi:** Nhận dạng, khai triển hình · Diện tích xung quanh, diện tích toàn phần · Thể tích · Đơn vị đo thể tích

| | Mã phiếu ôn lại |
|---|---|
| Tuyến 1 | `P082`, `P089` |
| Tuyến 2 | `P071`, `P092` |

### 3. Số đo thời gian  ·  nhóm **E — Đại lượng – Đo lường – Thời gian**

**Nội dung cốt lõi:** Bảng đơn vị đo thời gian · Cộng, trừ, nhân, chia số đo thời gian

| | Mã phiếu ôn lại |
|---|---|
| Tuyến 1 | `P059`, `P073`, `P085`, `P096` |
| Tuyến 2 | `P069`, `P073`, `P079`, `P097` |

### 4. Toán chuyển động đều  ·  nhóm **D — Toán điển hình & Giải toán có lời văn**

**Nội dung cốt lõi:** Vận tốc, quãng đường, thời gian · Chuyển động cùng chiều, ngược chiều · Chuyển động trên dòng nước

| | Mã phiếu ôn lại |
|---|---|
| Tuyến 1 | `P055`, `P076`, `P087` |
| Tuyến 2 | `P057`, `P072`, `P091` |

### 5. Ôn tập cuối cấp tiểu học  ·  nhóm **D — Toán điển hình & Giải toán có lời văn**

**Nội dung cốt lõi:** Bốn phép tính với số tự nhiên, phân số, số thập phân · Toán điển hình · Hình học phẳng và không gian · Biểu đồ và số liệu

| | Mã phiếu ôn lại |
|---|---|
| Tuyến 1 | `P081`, `P087`, `P094`, `P098`, `P099` |
| Tuyến 2 | `P078`, `P085`, `P087`, `P091`, `P099` |

> Mã phiếu viết tắt: `P0xx` thuộc khối `GITA-T1-L5` hoặc `GITA-T2-L5`.
> Khi một mạch không có phiếu nào khớp từ khoá trong kỳ này, bảng liệt kê toàn bộ phiếu cùng nhóm chuyên đề của kỳ để huấn luyện viên tự chọn.

---

## 3. CÔNG THỨC VÀ QUY TẮC PHẢI THUỘC

| # | Mạch | Phải thuộc lòng |
|---:|---|---|
| 1 | Hình tròn | Chu vi = đường kính × 3,14 |
| 2 | Hình tròn | Diện tích = bán kính × bán kính × 3,14 |
| 3 | Hình hộp chữ nhật và hình lập phương | S xung quanh hộp chữ nhật = chu vi đáy × chiều cao |
| 4 | Hình hộp chữ nhật và hình lập phương | S toàn phần = S xung quanh + 2 lần S đáy |
| 5 | Hình hộp chữ nhật và hình lập phương | V hộp chữ nhật = dài × rộng × cao |
| 6 | Hình hộp chữ nhật và hình lập phương | V lập phương = cạnh × cạnh × cạnh |
| 7 | Hình hộp chữ nhật và hình lập phương | 1 dm khối = 1 000 cm khối = 1 lít |
| 8 | Số đo thời gian | 1 giờ = 60 phút = 3 600 giây |
| 9 | Số đo thời gian | 1 ngày = 24 giờ |
| 10 | Toán chuyển động đều | v = s : t |
| 11 | Toán chuyển động đều | s = v × t |
| 12 | Toán chuyển động đều | t = s : v |
| 13 | Toán chuyển động đều | Ngược chiều: thời gian gặp nhau = quãng đường : tổng hai vận tốc |
| 14 | Toán chuyển động đều | Cùng chiều: thời gian đuổi kịp = khoảng cách : hiệu hai vận tốc |
| 15 | Ôn tập cuối cấp tiểu học | Toàn bộ công thức đã học ở lớp 4 và lớp 5 |

---

## 4. LỖI KINH ĐIỂN — ĐỌC TRƯỚC MỖI KỲ THI

| Mạch | Lỗi thường mắc |
|---|---|
| Hình tròn | Dùng đường kính thay bán kính khi tính diện tích |
| Hình hộp chữ nhật và hình lập phương | Quên nhân 2 lần diện tích đáy khi tính diện tích toàn phần |
| Số đo thời gian | Cộng phút quá 60 mà không đổi sang giờ |
| Toán chuyển động đều | Không đổi đơn vị thời gian về cùng loại trước khi tính |
| Ôn tập cuối cấp tiểu học | Trình bày thiếu lời giải và thiếu đáp số |

---

## 5. CHECKLIST TỰ KIỂM — “EM ĐÃ CHẮC CHƯA?”

Trả lời được ngay trong 10 giây thì đánh ✔. Nếu phải nghĩ lâu, đánh ✘
và làm lại các phiếu ở mục 2.

- [ ] **1.** (Hình tròn) Bán kính 2 cm thì diện tích bằng bao nhiêu?
- [ ] **2.** (Hình hộp chữ nhật và hình lập phương) Hộp 5 × 4 × 3 cm có thể tích bao nhiêu?
- [ ] **3.** (Số đo thời gian) 2 giờ 45 phút cộng 1 giờ 30 phút bằng bao nhiêu?
- [ ] **4.** (Toán chuyển động đều) Đi 45 km trong 1 giờ 30 phút thì vận tốc bằng bao nhiêu?
- [ ] **5.** (Ôn tập cuối cấp tiểu học) Em viết được đủ lời giải, phép tính và đáp số cho mỗi bài chưa?

**Chuẩn đạt:** ✔ từ 4/5 câu trở lên mới được coi là vững kiến thức học kỳ ii.

---

## 6. LỘ TRÌNH ÔN THEO BẢN ĐỒ (4 BUỔI)

| Buổi | Việc làm | Sản phẩm |
|:--:|---|---|
| 1 | Đọc cây kiến thức, tự chấm checklist mục 5 | Danh sách mạch còn ✘ |
| 2 | Làm lại các phiếu ở mục 2 ứng với mạch ✘ | Sổ lỗi cập nhật |
| 3 | Học thuộc bảng công thức mục 3, kiểm tra chéo trong team | Bảng công thức không nhìn sách |
| 4 | Làm 1 đề trong bộ đề mốc tương ứng (`07-de-thi/`) | Điểm và bảng phân tích lỗi |

