---
ma: "GITA-T1-L3-C08-HD"
tuyen: "T1"
lop: 3
cum: 8
cum_ten: "Một phần mấy, tỉ số và thống kê"
loai: "HD"
loai_ten: "Phiếu Hướng dẫn ôn chắc chuyên đề"
nhom_ma: "H"
nhom_ten: "Phân số – Số thập phân – Tỉ số phần trăm – Thống kê"
ten: "Một phần mấy, tỉ số và thống kê — Hướng dẫn ôn chắc chuyên đề"
hoc_ky: "HK1"
tuan: 15
---

# PHIẾU HƯỚNG DẪN ÔN CHẮC CHUYÊN ĐỀ · GITA-T1-L3-C08-HD

**HỌC VIỆN PHÁT TRIỂN TÀI NĂNG TOÀN CẦU — GITA** · *Tư duy xuất sắc, Bản lĩnh dẫn đầu* · gita.edu.vn
Tuyến 1 — Nền tảng đến Nâng cao · Lớp 3 · Tuần 15 · HK1
**Cụm chuyên đề C08 — Một phần mấy, tỉ số và thống kê**
Nhóm chuyên đề **H — Phân số – Số thập phân – Tỉ số phần trăm – Thống kê**
Phiếu này dùng để **tự ôn chắc cả cụm** sau khi đã học xong sáu buổi. Không cần giáo viên nhắc.

---

## 1. BẢN ĐỒ CHƯƠNG

```
CỤM C08 — Một phần mấy, tỉ số và thống kê
├── Một phần mấy của một số — nhận biết và tìm
├── Bảng thống kê số liệu — đọc và trả lời câu hỏi
├── Biểu đồ tranh — đọc và lập
├── Tìm một phần mấy của một số trong bài toán có lời văn
├── Tỉ số đơn giản: gấp mấy lần, bằng một phần mấy
├── Đọc và phân tích bảng số liệu kép
└── Ôn tập phân số, tỉ số và thống kê
```

| Buổi | Loại phiếu | Mã phiếu | Sản phẩm phải có sau buổi |
|:--:|---|---|---|
| 1 | Lý thuyết | `GITA-T1-L3-C08-LT` | Sơ đồ tư duy chương tự vẽ |
| 2 | Dạng bài & Đọc vị | `GITA-T1-L3-C08-DB` | Bảng dạng bài — dấu hiệu điền đủ |
| 3 | Kỹ năng & Phương pháp | `GITA-T1-L3-C08-KN` | Sổ lỗi và kịch bản thuyết trình |
| 4 | Luyện nâng cao | `GITA-T1-L3-C08-NC` | Bài IV, V đã chữa và ghi lỗi |
| 5 | Ôn thi | `GITA-T1-L3-C08-OT` | Điểm bài ôn và bảng phân tích lỗi |
| 6 | Thi chương | `GITA-T1-L3-C08-TH` | Điểm bài thi chương |

---

## 2. BẢNG CÔNG THỨC VÀ QUY TẮC PHẢI THUỘC

Học thuộc theo cặp **dạng bài – công thức**, không học rời.

| # | Dùng cho dạng bài | Công thức hoặc quy tắc |
|---:|---|---|
| 1 | Tìm phân số của một số qua nhiều bước | Phân số một phần mấy, phép trừ liên tiếp |
| 2 | Tìm một phần mấy của một số | Phân số dạng một phần mấy |
| 3 | Tìm một phần mấy rồi tính phần còn lại | Phân số một phần mấy, phép trừ |
| 4 | Đọc bảng số liệu | Thống kê đơn giản, trung bình cộng |
| 5 | Tìm số khi biết một phần mấy của nó | Quan hệ ngược của phép chia |

---

## 3. BẢNG DẠNG BÀI VÀ DẤU HIỆU NHẬN BIẾT

| Dạng bài | Dữ liệu nhận biết | Phương pháp áp dụng | Cách xử lý nhanh nhất |
|---|---|---|---|
| Tìm phân số của một số qua nhiều bước | Cụm “… số còn lại” | Tính tuần tự, ghi lại số còn lại sau mỗi bước | Lấy đi 1 phần m thì còn (m − 1) phần m — nhân trực tiếp cho nhanh. |
| Tìm một phần mấy của một số | Cụm “một phần mấy của …” | Chia số đó cho mẫu số | Kết quả luôn nhỏ hơn số ban đầu — dùng để kiểm tra ngay. |
| Tìm một phần mấy rồi tính phần còn lại | “Đã bán một phần mấy”, hỏi “còn lại” | Chia để tìm phần đã dùng rồi trừ | Bán 1 phần 4 thì còn 3 phần 4 — chia rồi nhân 3, một bước là xong. |
| Đọc bảng số liệu | Đề cho bảng số liệu kèm nhiều câu hỏi | Ghi lại số liệu rồi trả lời từng câu | Tính tổng một lần rồi dùng lại cho các câu sau. |
| Tìm số khi biết một phần mấy của nó | “Một phần mấy của một số là …, tìm số đó” | Nhân giá trị một phần với mẫu số | Kết quả phải lớn hơn số đã cho — dùng để loại đáp số sai ngay. |

---

## 4. LỘ TRÌNH ÔN BỐN BUỔI

| Buổi ôn | Việc làm | Thời lượng | Sản phẩm |
|:--:|---|:--:|---|
| 1 | Đọc lại bản đồ chương ở mục 1, tự vẽ lại sơ đồ tư duy cụm C08 mà không nhìn phiếu | 45 phút | Sơ đồ tư duy tự vẽ |
| 2 | Học thuộc bảng công thức mục 2, kiểm tra chéo trong nhóm hai bạn | 45 phút | Đọc thuộc không nhìn |
| 3 | Làm lại phiếu `GITA-T1-L3-C08-NC` phần IV và V, chấm bằng phiếu `GITA-T1-L3-C08-NC-GP` | 60 phút | Sổ lỗi cập nhật |
| 4 | Làm lại phiếu `GITA-T1-L3-C08-TH` trong đúng 90 phút, tự chấm | 90 phút | Điểm và bảng phân tích lỗi |

---

## 5. CHECKLIST TỰ KIỂM

Trả lời được ngay trong 10 giây thì đánh ✔. Phải nghĩ lâu thì đánh ✘ và làm lại phần tương ứng.

- [ ] **1.** Em nêu được dấu hiệu nhận biết của dạng “Tìm phân số của một số qua nhiều bước” chứ?
- [ ] **2.** Em nhớ công thức: Phân số một phần mấy, phép trừ liên tiếp?
- [ ] **3.** Em nêu được dấu hiệu nhận biết của dạng “Tìm một phần mấy của một số” chứ?
- [ ] **4.** Em nhớ công thức: Phân số dạng một phần mấy?
- [ ] **5.** Em nêu được dấu hiệu nhận biết của dạng “Tìm một phần mấy rồi tính phần còn lại” chứ?
- [ ] **6.** Em nhớ công thức: Phân số một phần mấy, phép trừ?
- [ ] **7.** Em nêu được dấu hiệu nhận biết của dạng “Đọc bảng số liệu” chứ?
- [ ] **8.** Em nhớ công thức: Thống kê đơn giản, trung bình cộng?
- [ ] **9.** Em nêu được dấu hiệu nhận biết của dạng “Tìm số khi biết một phần mấy của nó” chứ?
- [ ] **10.** Em nhớ công thức: Quan hệ ngược của phép chia?

**Chuẩn đạt:** ✔ từ 9/10 câu trở lên.

---

## 6. SỔ LỖI MẪU

Đây là những lỗi học viên khoá trước mắc nhiều nhất ở cụm này. Chép vào sổ lỗi của em, kèm cột cách phòng.

| # | Lỗi thường gặp | Cách phòng |
|---:|---|---|
| 1 | Lấy cả hai phân số tính trên tổng ban đầu. | Sau mỗi bước, viết rõ “còn lại … quyển” rồi mới sang bước sau. |
| 2 | Nhân thay vì chia. | Kiểm tra: kết quả phải **bé hơn** số ban đầu. |
| 3 | Dừng ở bước một, trả lời số gạo đã bán. | Đọc lại câu hỏi sau khi tính xong bước một. |
| 4 | Đọc lệch cột nên gán nhầm số cho bạn khác. | Dùng ngón tay dò theo cột khi đọc bảng. |
| 5 | Chia thay vì nhân. | Kiểm tra: số tìm được phải **lớn hơn** số đã cho. |

---

## 7. TIÊU CHÍ ÔN CHẮC

Cụm này chỉ được coi là **đã ôn chắc** khi đủ cả bốn điều kiện sau.

| # | Tiêu chí | Ngưỡng đạt |
|---:|---|---|
| 1 | Điểm phiếu thi chương `GITA-T1-L3-C08-TH` | ≥ 80/100 |
| 2 | Checklist tự kiểm ở mục 5 | ≥ 90% số câu đánh ✔ |
| 3 | Bảng công thức mục 2 | Đọc thuộc, không nhìn sách |
| 4 | Sổ lỗi | Mỗi lỗi ở mục 6 đều có một dòng cách phòng do em tự viết |

Chưa đạt thì **học lại phiếu `GITA-T1-L3-C08-NC`** và làm bài tương tự trong phiếu `GITA-T1-L3-C08-NC-GP`, chưa mở cụm mới.

---

**Người biên soạn:** Ban chuyên môn Học viện GITA
**Phiên bản:** 2.0 · Chuẩn biên soạn phiếu GITA v2.0 · Đáp số do bộ sinh nội dung GITA tính, đã đối chiếu tự động.
