...
import { useGitaProfile, type GitaView } from '../../gita/useGitaProfile.ts';
...
export function GitaHome(): React.ReactElement {
  ...
  return (
    <div className="page stack gap-6">
      ...
      <GitaHero view={view} locale={locale} />
      ...
      {active === 'profile' && <ProfilePanel view={view} locale={locale} />}
...
/*
 * `GitaHero` and `ProfilePanel` live here, at module scope, and not inside
 * `GitaHome`.
 *
 * They were declared inside it, which is the one thing this codebase forbids
 * outright. A component defined in another component's body gets a new
 * identity on every parent render, so React unmounts and remounts the whole
 * subtree. With a store-wide reducer that means every dispatch — logging a
 * habit, switching a tab, any autopilot tick — destroyed whatever was on
 * screen and rebuilt it. Nothing errored; the panel simply flickered and lost
 * its scroll position, and any local state it grew later would have vanished
 * silently.
 *
 * Lifted out, they take their data as props.
 */

function GitaHero({ view, locale }: { view: GitaView; locale: Locale }): React.ReactElement {
...
function ProfilePanel({ view, locale }: { view: GitaView; locale: Locale }): React.ReactElement {
